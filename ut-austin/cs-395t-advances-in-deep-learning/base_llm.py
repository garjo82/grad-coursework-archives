...
class BaseLLM:
    ...
    def batched_generate(
        ...
        # Setup tokenizer for left-padding batched generation
        ...
        # Build generation kwargs dictionary
        gen_kwargs = {
            "max_new_tokens": 50,
        ...
        # Strict inference mode to save VRAM
        with torch.inference_mode():
            outputs = self.model.generate(**inputs, **gen_kwargs)

        # Isolate generated tokens by skipping prompt length
        input_length = inputs["input_ids"].shape[1]
        generated_tokens = outputs[:, input_length:]

        # Decode only outputs without special tokens
        decoded_outputs = self.tokenizer.batch_decode(
            generated_tokens, skip_special_tokens=True
        )

        # Reshape output matrix based on required sequences
        if num_return_sequences is not None:
            return [
